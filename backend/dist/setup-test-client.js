...
async function setupTestClient() {
    ...
    const { data: clients } = await supabase_1.supabase.from('clients').select('*').limit(1);
    if (!clients || clients.length === 0) {
        console.log('Creating new test client...');
        await supabase_1.supabase.from('clients').insert({
        ...
        if (error)
            ...
            console.log('✅ Client updated with all credentials (Meta + Google + GA4 + Sheets + Refresh Token)!');
    }
}
setupTestClient();
